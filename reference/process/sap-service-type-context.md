...
**Dung o dau**: goi tu skill `sap-routing-discipline` (rule R9), skill `sap-ask-consultant`,
skill `sap-extensibility`, skill `sap-clean-code`, va skill `sap-abap-sql` — ca 5 deu tro ve
day bang ten truoc khi tra loi 1 cau hoi bi anh huong boi edition (s4hc_(private)/
s4hc_(public)/btp/onprem) ma chua biet trong phien hien tai. Chuyen tu
`skills/sap-service-type-context/` sang day 2026-07-31 (xem
`docs/audits/2026-Q3-skill-consolidation-part2.md`) — noi dung khong doi, chi khong con tu
trigger qua tu khoa nua: skill/agent can no phai chu dong `Read` file nay theo dung ten.

Khac truc voi `sap-deployment-target` (xac dinh package deploy) va `sap-bootstrap-system-context`
(xac dinh quy uoc dat ten thuc te) — 3 tai lieu/skill co the chay noi tiep: tai lieu nay
(edition) -> `sap-bootstrap-system-context` (quy uoc that) -> `sap-deployment-target` (package).
...
- ❌ Can biet **quy uoc dat ten thuc te** (Domain/Data Element tai su dung duoc) -> dung
  `sap-bootstrap-system-context`. Co the chay **sau** tai lieu nay (biet edition truoc, do
  quy uoc thuc te sau).
...
- 🔗 `sap-routing-discipline` (R9) tro toi tai lieu nay truoc khi tra loi cau hoi anh huong boi
  edition.
- 🔗 `sap-ask-consultant`, `sap-extensibility`, `sap-clean-code`, `sap-abap-sql` doc ket qua
  tai lieu nay de rẽ nhanh/canh bao thay vi mac dinh Public Edition.
