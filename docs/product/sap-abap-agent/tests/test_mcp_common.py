...
def test_list_servers_returns_name_command_args_env():
    obj = {"mcpServers": {
        "sap-vsp": {"type": "stdio", "command": "/bin/vsp", "args": ["mcp"], "env": {"SAP_ADT_URL": "x"}},
    }}
    result = mc.list_servers(obj)
    assert len(result) == 1
    info = result[0]
    assert info.name == "sap-vsp"
    assert info.command == "/bin/vsp"
    assert info.args == ["mcp"]
    assert info.env == {"SAP_ADT_URL": "x"}
    assert info.url is None


def test_list_servers_handles_sse_url_server():
    obj = {"cds-kb": {"type": "sse", "url": "https://example.com/sse"}}
    result = mc.list_servers(obj)
...
def test_list_servers_empty_object():
    assert mc.list_servers({}) == []


def test_list_servers_skips_non_dict_values():
    obj = {"mcpServers": {"broken": "not-a-dict", "ok": {"command": "x"}}}
    result = mc.list_servers(obj)
    assert len(result) == 1
    assert result[0].name == "ok"


def test_list_servers_multiple_servers_including_sap_vsp():
    obj = {"mcpServers": {
        "sap-connect": {"type": "stdio", "command": "mcp-sap-connect"},
        "sap-vsp": {"type": "stdio", "command": "/bin/vsp", "args": ["mcp"]},
    }}
    names = [s.name for s in mc.list_servers(obj)]
    assert "sap-vsp" in names
    assert "sap-connect" in names
