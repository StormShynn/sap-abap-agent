...
| R9 | "Cau hoi dung cham data access/extensibility/cu phap ABAP Cloud, chua ro he thong la edition nao" | Doc `reference/process/sap-service-type-context.md` truoc (s4hc_(private)/s4hc_(public)/btp/onprem), tru khi da xac dinh trong phien nay - tranh mac dinh Public Edition sai ngu canh |
...
- Skill `sap-ask-before-guessing` - nguyen tac song song, bom qua cung hook: TONG QUAT hoa vien
  ...
  routing module.
- `reference/process/sap-service-type-context.md` (R9) - xac dinh edition (s4hc_(private)/
  s4hc_(public)/btp/onprem) truoc khi tra loi cau hoi bi anh huong boi edition; doc offline tu
  config cua mcp-sap-connect, hoi user neu chua xac dinh duoc.